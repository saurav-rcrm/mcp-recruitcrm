...
| `get_job_assigned_candidates` | Fetch assigned candidates for one job and return compact assignment summaries with `candidate_slug`, stage date, and stage label/id. |
| `list_candidate_hiring_stages` | List compact candidate hiring stage rows to resolve labels such as `Placed` to ids used by `get_job_assigned_candidates.status_id`. |
...
| --- | --- | --- |
| Candidate | `https://app.recruitcrm.io/candidate/{slug}` | `search_candidates.slug`, `get_candidate_details.slug`, or `get_job_assigned_candidates.assigned_candidates[].candidate_slug` |
...
- `get_job_assigned_candidates` excludes the raw nested candidate payload and exposes only compact assignment fields needed for job-stage workflows
- `list_candidate_hiring_stages` exposes only compact `stage_id` and `label` rows from the hiring pipeline payload
